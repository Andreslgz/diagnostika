...
<script>
      ...
      switch (filterType) {
        ...
        case 'mes':
          ...
          meses.forEach((mes, idx) => {
            ...
            mesOptions += `<option value="${val}" ${selected}>${mes}</option>`;
          });

          // Años (5 atrás y 5 adelante)
          let yearOptions = "";
          for (let y = currentYear - 5; y <= currentYear + 5; y++) {
  ...
  <script>
    document.addEventListener('DOMContentLoaded', () => {
      ...
      function renderInputs(filter) {
        ...
        if (btnBuscar) btnBuscar.addEventListener('click', (ev) => {
          ev.preventDefault();
          fetchAndRender();
        });
      }
      ...
      async function fetchAndRender() {
        ...
        try {
          ...
          try {
            ...
            /* intenta extraer JSON si hay BOM/debug */
            const s = raw.indexOf('{'),
              e = raw.lastIndexOf('}');
            if (s > -1 && e > s) {
              try {
  ...
  <script>
    ...
    function renderOrders(rows) {
      const container = document.querySelector(".border.border-solid.border-gray-400");
      if (!rows.length) {
        container.innerHTML = "<p class='text-gray-500'>No se encontraron órdenes.</p>";
        return;
      }
