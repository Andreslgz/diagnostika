...
if ($accion === 'eliminar' && $sl_id) {
    // Buscar para borrar imágenes físicas (desktop y móvil)
    $row = $database->get("slider", ["sl_img", "sl_img_mov"], ["sl_id" => $sl_id]);
    $del = $database->delete("slider", ["sl_id" => $sl_id]);
    if ($del->rowCount() > 0) {
        borrarImagenSlider($row['sl_img'] ?? null);
        borrarImagenSlider($row['sl_img_mov'] ?? null);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['form'] ?? '') === 'slider') {
    ...
    // Si viene imagen móvil, procesar
    if (!empty($_FILES['sl_img_mov']['name'])) {
        [$ok2, $nombreGuardado2, $err2] = moverImagenSlider($_FILES['sl_img_mov']);
        if (!$ok2) {
            // Si falló móvil y subimos desktop nueva, la borramos para no dejar basura
            if ($sl_img_nueva) borrarImagenSlider($sl_img_nueva);
            $_SESSION['resultado'] = 'Error de imagen (móvil): ' . $err2;
            header("Location: sliders.php");
            exit;
        }
        $sl_img_mov_nueva = $nombreGuardado2;
    }
    ...
    if (isset($_POST['editar']) && $_POST['editar'] == "1") {
        if (!$sl_id) {
            if ($sl_img_nueva) borrarImagenSlider($sl_img_nueva);
            if ($sl_img_mov_nueva) borrarImagenSlider($sl_img_mov_nueva);
            $_SESSION['resultado'] = 'ID de slider no válido.';
            header("Location: sliders.php");
            exit;
        }
        // Obtener imágenes anteriores por si hay que borrarlas
        $anterior = $database->get("slider", ["sl_img", "sl_img_mov"], ["sl_id" => $sl_id]);
        ...
        try {
            $ins = $database->insert("slider", [
                "sl_tit"     => $sl_tit,
                "sl_img"     => $sl_img_nueva,     // puede ser null si permites crear sin imagen
                "sl_img_mov" => $sl_img_mov_nueva, // idem
                "sl_est"     => $sl_est
            ]);
...
<head>
    ...
    <!-- COLOR SKIN CSS -->
    <link id="theme" rel="stylesheet" type="text/css" media="all" href="<?php echo $url; ?>/panel/assets/colors/color1.css" />
    <script>
    ...
    Object.entries(sashConfig).forEach(([key, value]) => localStorage.setItem(key, value));
    </script>
</head>
...
<body class="app sidebar-mini ltr light-mode">
    ...
    <div class="page">
                                                ...
                                                <div class="col-lg-12 col-md-12">
                                                    <div class="row">
                                                        ...
                                                        <div class="text-end mb-3">
                                                            <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modalAgregarSlider">
                                                        ...
                                                        <table class="table border text-nowrap text-md-nowrap table-bordered mb-0">
                                                            ...
                                                            <tbody>
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                <?php if (!empty($s['sl_img'])): ?>
                                                                                    <img src="../uploads/slider/<?= h($s['sl_img']) ?>" alt="" style="height:42px;border-radius:6px;">
                                                                                <?php else: ?>
                                                                                    <span class="text-muted">Sin imagen</span>
                                                                                <?php endif; ?>
                                                                            </td>
                                                                            <td>
                                                                                <?php if (!empty($s['sl_img_mov'])): ?>
                                                                                    <img src="../uploads/slider/<?= h($s['sl_img_mov']) ?>" alt="" style="height:42px;border-radius:6px;">
                                                                                <?php else: ?>
                                                                                    <span class="text-muted">Sin imagen</span>
                                                                                <?php endif; ?>
                                                                            </td>
                                                                ...
                                                                <?php else: ?>
                                                                    <tr>
                                                                        <td colspan="6" class="text-center text-muted">No se encontraron slides.</td>
                                                                    </tr>
                                                                <?php endif; ?>
                                                            </tbody>
                                                        </table>
        ...
        <!-- MODAL: Agregar -->
        <div class="modal fade" id="modalAgregarSlider" tabindex="-1" aria-labelledby="modalAgregarSliderLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content border-0 shadow rounded-3">
                    <form id="formNuevoSlider" method="POST" action="sliders.php" enctype="multipart/form-data" novalidate>
                        <input type="hidden" name="form" value="slider">

                        <div class="modal-header border-bottom">
                            <h5 class="modal-title fw-semibold text-dark" id="modalAgregarSliderLabel">Agregar Slide</h5>
                        ...
                        <div class="modal-body text-muted">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="sl_tit" placeholder="Ej. Promoción de verano" required>
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-semibold">Imagen Desktop (jpg, png, webp, gif)</label>
                                <input type="file" class="form-control" name="sl_img" accept=".jpg,.jpeg,.png,.webp,.gif">
                                <small class="text-muted">Máx 5MB</small>
                            </div>

                            <div class="mb-3">
                                <label class="form-label fw-semibold">Imagen Móvil (jpg, png, webp, gif)</label>
                                <input type="file" class="form-control" name="sl_img_mov" accept=".jpg,.jpeg,.png,.webp,.gif">
                                <small class="text-muted">Máx 5MB</small>
                            </div>
        ...
        <!-- MODAL: Editar -->
        <div class="modal fade" id="modalEditarSlider" tabindex="-1" aria-labelledby="modalEditarSliderLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content border-0 shadow rounded-3">
                    <form id="formEditarSlider" method="POST" action="sliders.php" enctype="multipart/form-data" novalidate>
                        <input type="hidden" name="form" value="slider">
                        ...
                        <div class="modal-body text-muted">
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-semibold">Imagen actual Desktop</label>
                            ...
                            <div class="mb-3">
                                <label class="form-label fw-semibold">Imagen actual Móvil</label>
                                <div id="edit_preview_img_mov" class="mb-2"></div>
                                <label class="form-label fw-semibold">Cambiar imagen (opcional)</label>
                                <input type="file" class="form-control" name="sl_img_mov" accept=".jpg,.jpeg,.png,.webp,.gif">
                            </div>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        if (modalDel) {
            modalDel.addEventListener('show.bs.modal', function (event) {
                ...
                document.getElementById('textoEliminar').innerHTML = `¿Eliminar el slide <strong>${tit}</strong>?`;
                document.getElementById('btnEliminarConfirmado').href = `sliders.php?ac=eliminar&id=${id}`;
            });
        }
        ...
        document.querySelectorAll(".btn-editar-slider").forEach(button => {
            button.addEventListener("click", function () {
                ...
                const imgMov  = this.getAttribute("data-img-mov") || '';
                ...
                const contMov = document.getElementById("edit_preview_img_mov");
                contMov.innerHTML = imgMov
                    ? `<img src="../uploads/slider/${imgMov}" alt="" style="height:60px;border-radius:6px;">`
                    : `<span class="text-muted">Sin imagen</span>`;
